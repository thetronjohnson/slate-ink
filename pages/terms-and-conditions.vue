<template>
  ...
  // Update query to fetch terms and conditions content
  const { data: doc } = await useAsyncData('terms', () => queryContent('/terms-and-conditions').findOne())
  
  useHead({
    title: computed(() => `${doc.value?.title || 'Terms and Conditions'} - Slate`),
    meta: [
      {
        name: 'description',
        content: computed(() => doc.value?.description || 'Terms and Conditions for Slate - Understanding your rights and obligations when using our service.')
      },
      {
        property: 'og:title',
        content: computed(() => `${doc.value?.title || 'Terms and Conditions'} - Slate`)
      },
      {
      ...
      {
        property: 'og:url',
        content: `${config.public.siteUrl}/terms-and-conditions`
      },
      {
        name: 'robots',
