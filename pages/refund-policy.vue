<template>
  ...
  // Update query to fetch refund policy instead of privacy
  const { data: doc } = await useAsyncData('refund', () => queryContent('/refund-policy').findOne())
  
  useHead({
    title: computed(() => `${doc.value?.title || 'Refund Policy'} - Slate`),
    meta: [
      {
        name: 'description',
        content: computed(() => doc.value?.description || 'Refund Policy for Slate - Learn about our refund terms and conditions.')
      },
      {
        property: 'og:title',
        content: computed(() => `${doc.value?.title || 'Refund Policy'} - Slate`)
      },
      {
      ...
      {
        property: 'og:url',
        content: `${config.public.siteUrl}/refund-policy`
      },
      {
        name: 'robots',
